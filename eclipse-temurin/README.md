...
[![arm32v6/eclipse-temurin build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/eclipse-temurin.svg?label=arm32v6/eclipse-temurin%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/eclipse-temurin/)
...
```console
$ docker run -v $(pwd)/certs:/certificates/ -e USE_SYSTEM_CA_CERTS=1 arm32v6/eclipse-temurin:11
```
...
```dockerfile
FROM arm32v6/eclipse-temurin:11
RUN mkdir /opt/app
...
COPY --from=arm32v6/eclipse-temurin:11 $JAVA_HOME $JAVA_HOME
...
FROM arm32v6/eclipse-temurin:11 as jre-build
...
```dockerfile
FROM arm32v6/eclipse-temurin:11.0.12_7-jdk
CMD ["java", "-jar", "/opt/app/japp.jar"]
```
