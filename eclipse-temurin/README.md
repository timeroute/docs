...
[![mips64le/eclipse-temurin build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/mips64le/job/eclipse-temurin.svg?label=mips64le/eclipse-temurin%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/mips64le/job/eclipse-temurin/)
...
```console
$ docker run -v $(pwd)/certs:/certificates/ -e USE_SYSTEM_CA_CERTS=1 mips64le/eclipse-temurin:11
```
...
```dockerfile
FROM mips64le/eclipse-temurin:11
RUN mkdir /opt/app
...
COPY --from=mips64le/eclipse-temurin:11 $JAVA_HOME $JAVA_HOME
...
FROM mips64le/eclipse-temurin:11 as jre-build
...
```dockerfile
FROM mips64le/eclipse-temurin:11.0.12_7-jdk
CMD ["java", "-jar", "/opt/app/japp.jar"]
```
