...
```console
$ docker run -v $(pwd)/certs:/certificates/ -e USE_SYSTEM_CA_CERTS=1 riscv64/eclipse-temurin:11
```
...
```dockerfile
FROM riscv64/eclipse-temurin:11
RUN mkdir /opt/app
...
COPY --from=riscv64/eclipse-temurin:11 $JAVA_HOME $JAVA_HOME
...
FROM riscv64/eclipse-temurin:11 as jre-build
...
```dockerfile
FROM riscv64/eclipse-temurin:11.0.12_7-jdk
CMD ["java", "-jar", "/opt/app/japp.jar"]
```
